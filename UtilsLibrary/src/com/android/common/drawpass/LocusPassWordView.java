...
public class LocusPassWordView extends View {
	...
	private long CLEAR_TIME = 600;
	public static final int PWD_MAX_LEN = 9;
	...
	private int selectedColor = 0xff11cd6e;
	private int outterSelectedColor = 0xff00CC66;
	private int outterErrorColor = 0xff901032;
	private int dotColor = 0xff66cd6e;
...
//			if ((dx > 1 || dy > 1) && (dx != 2 * dy) && (dy != 2 * dx)) {
	...
	private String toPointString() {
		if (sPoints.size() >= PWD_MIN_LEN && sPoints.size() <= PWD_MAX_LEN) {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (isFinish) {
			if (this.sPoints.size() == 1) {
				this.reset();
			} else if (sPoints.size() < PWD_MIN_LEN || sPoints.size() > PWD_MAX_LEN) {
				error();
				clearPassword();
				Toast.makeText(this.getContext(), R.string.utilslib_gesturepass_pass2short, Toast.LENGTH_SHORT).show();
