...
/**
 * @TiTle SlideSwitch.java
 * @Package com.android.common.baseui.views
 * @Description 切换开关（IOS样式）
 * @Date 2016年10月26日
 * @Author siyuan
 * @Refactor 
...
public class SlideSwitchView extends View {
	...
	private float default_text_size = 48f;
	...
	public SlideSwitchView(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.slideswitchview);
		color_theme = a.getColor(R.styleable.slideswitchview_slideswitchThemeColor, DEFAULT_COLOR_THEME);
		isOpen = a.getBoolean(R.styleable.slideswitchview_slideswitchIsOpen, false);
		shape = a.getInt(R.styleable.slideswitchview_slideswitchShape, SHAPE_RECT);
		mOpenText = a.getString(R.styleable.slideswitchview_slideswitchTextOpen);
		mCloseText = a.getString(R.styleable.slideswitchview_slideswitchTextClose);
		mTextSize = a.getDimension(R.styleable.slideswitchview_slideswitchTextSize, default_text_size);
		a.recycle();
		initView();
	}
	...
	private void drawText(Canvas canvas) {
		String[] text = {mOpenText, mCloseText};
		int width = getMeasuredWidth();
		int height = getMeasuredHeight();
		...
		for(int i = 0; i < text.length; i ++) {
			int fontWidth = (int) paint.measureText(text[i]);
			
			int startX = (i * width / 2) + (width / 2 - fontWidth)/2;
	        int startY = (int) (height/2 - (paint.ascent() + paint.descent())/2);
	        
	        canvas.drawText(text[i], startX, startY, paint);
		}
		paint.reset();
	}
