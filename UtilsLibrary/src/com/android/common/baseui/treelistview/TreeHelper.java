...
public class TreeHelper {
	...
	public static <T> List<Node> convertData2Nodes(List<T> datas, boolean isHide)
		...
		for (T t : datas) {
			...
			for (Field f : declaredFields) {
				...
				if ("loginName".equals(f.getName())) {
					f.setAccessible(true);
					loginName = (String) f.get(t);
				}
			...
			node = new Node(id, pId, name, loginName);
		...
		for (int i = 0; i < nodes.size(); i++) {
			...
			for (int j = i + 1; j < nodes.size(); j++) {
				Node m = nodes.get(j);
				if (n.getId().equalsIgnoreCase(m.getpId()) ) {
					n.getChildrenNodes().add(m);
					m.setParent(n);
				} else if (n.getpId().equalsIgnoreCase(m.getId())) {
					n.setParent(m);
	...
	public static void setNodeIcon(Node node) {
		if (node.getChildrenNodes().size() > 0 && node.isExpand()) {
			node.setIcon(R.drawable.utilslib_icon_arrawdown);//打开
		} else if (node.getChildrenNodes().size() > 0 && !node.isExpand()) {
			node.setIcon(R.drawable.utilslib_icon_arrowright);//关闭
		} else {			
			node.setIcon(-1);
		}
	}
